...
const getLabs = asyncHandler(async (req, res) => {
  const labs = await Lab.find({ createdBy: req.user._id }).sort({ createdAt: -1 });
  res.json({ success: true, data: labs });
});

// POST /api/instructor/labs
const createLab = asyncHandler(async (req, res) => {
  const { title, labNumber, instructions, dueDate, points, difficulty, languages, testCases, solutions } = req.body;
